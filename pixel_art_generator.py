...
Usa Variational Autoencoder para gerar imagens coerentes
"""
...
class VAEModel:
    """Carrega e usa o VAE treinado para geração."""
    def __init__(self):
    ...
    def sample_prior(self, seed=None):
        """Amostra do prior gaussiano (z ~ N(0,1))."""
    ...
    def generate_patch(self, noise_scale=1.0, seed=None):
        """Gera um patch 8x8 a partir de z amostrado."""
    ...
    def generate_image(self, width, height, noise_scale=1.0, seed=None,
                       coherence=0.3):
        """Gera imagem com patches coerentes entre si."""
        ...
        # Gera z_base para dar coerência global
        ...
        for py in range(n_py):
            for px in range(n_px):
                # Cada patch tem variação do z_base
...
def generate_pixel_art(prompt, width=64, height=64, style=None, palette_size=16,
    ...
    """Gera pixel art com VAE treinado."""
    print(f"\n🎨 Gerando: '{prompt}'")
    if model is None:
